...
BEGIN
    SELECT 
        ...
        IFNULL(
        (
            SELECT SUM(idat.po_amount)
            FROM btggasify_purchase_live.tbl_IRNReceipt_detail idat
            WHERE idat.poid = po.poid
              AND (gh.grnid IS NULL 
                   OR CAST(idat.grn_id AS UNSIGNED) = gh.grnid)
              AND (idtl.receiptnote_hdr_id IS NULL 
                   OR idat.receiptnote_hdr_id = idtl.receiptnote_hdr_id)
        ),
        (
            SELECT SUM(gdt.TotalAmount)
            FROM btggasify_purchase_live.tbl_grn_detail gdt
            WHERE gdt.poid = po.poid
              AND (gh.grnid IS NULL OR gdt.grnid = gh.grnid)
        )
        ) AS irn_amount,
    ...
    LEFT JOIN 
    (
        SELECT DISTINCT grnid, poid 
        FROM btggasify_purchase_live.tbl_grn_detail
    ) gd 
        ON gd.poid = po.poid

    LEFT JOIN btggasify_purchase_live.tbl_grn_header gh 
        ON gh.grnid = gd.grnid

    LEFT JOIN btggasify_purchase_live.tbl_IRNReceipt_detail idtl 
        ON idtl.poid = po.poid 
       AND (gh.grnid IS NULL 
            OR CAST(idtl.grn_id AS UNSIGNED) = gh.grnid)

    LEFT JOIN btggasify_finance_live.tbl_claimAndpayment_Details cdet 
        ON cdet.poid = po.poid 
       AND cdet.IsActive = 1

    LEFT JOIN btggasify_finance_live.tbl_claimAndpayment_header clm 
        ON (clm.irnid = idtl.receiptnote_hdr_id 
            AND clm.irnid IS NOT NULL)
        OR 
           (clm.Claim_ID = cdet.Claim_ID 
            AND clm.irnid IS NULL)
